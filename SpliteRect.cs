...
namespace PicSplit
{
    ...
    class SpliteRectGroup
    {
        ...
        public void SetRect(int widthSrc, int heightSrc, int startX, int startY,
            int widthDest, int heightDest,int heightDel)
        {
            _widthSrc = widthSrc;
            _heightSrc = heightSrc;
            _listSplitRect.Clear();
            _heightDel=heightDel;
        ...
        public void Draw(Graphics g)
        {
            ...
            foreach (Rectangle rect in _listSplitRect)
            {
                ...
                rectTmp.X = rect.X;
                rectTmp.Y = rect.Y + rect.Height - _heightDel;
                rectTmp.Width = rect.Width;
                rectTmp.Height = _heightDel;
                g.DrawRectangle(Pens.Blue , rectTmp);
            }
            strfont.Dispose();
            hat1.Dispose();
            hat2.Dispose();
